    ...
    <div className="w-full max-h-[calc(100vh-6rem)] overflow-y-scroll no-scrollbar bg-white/50 dark:bg-popover/15 h-full">
      <div className="grid grid-cols-4 p-2 gap-1.5">
        <DashboardCard>
          ...
          <DashboardCardContent className="items-start justify-end">
            <SummaryChart1 />
        ...
        <DashboardCard className="col-span-1">
          ...
          <div className="flex w-full max-h-96 overflow-hidden flex-col items-start justify-start gap-1">
            {Array.from({ length: 6 }).map((item, idx) => {
              return (
                <Item
                  variant="outline"
                  size="sm"
                  className="w-full rounded-md border bg-white/40 dark:bg-black/25 px-3 py-2 h-16"
                  key={idx}
                >
                  <ItemMedia>
