...
export function ListContent() {
  return (
    <div className="w-full h-[calc(100vh-6rem)] overflow-y-scroll no-scrollbar bg-muted/35 dark:bg-popover/15 p-2 grid grid-cols-1 gap-1.5">
      <DashboardCard className="h-min">
        <div className="flex items-center justify-between w-full">
          <DashboardCardLabel className="hover:text-foreground transition-colors cursor-default">
            <ChevronDown />
            In Progress
          </DashboardCardLabel>
          <Badge
          ...
          {Array.from({ length: 6 }).map((item, idx) => {
            return (
              ...
              >
                ...
                <Badge
                  variant={"destructive"}
                  className="rounded-sm bg-transparent text-destructive border-destructive border"
                >
                  High
                </Badge>
